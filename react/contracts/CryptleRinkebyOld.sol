...
//VRF RELATED OBJECTS
    ...
    function signUp() public payable {
        ...
        require(msg.value >= 0.01 ether, "Error: INSUFFICIENT FUNDS PROVIDED");
        require(!enabled[msg.sender], "Error: PLAYER ALREADY SIGNED UP");
        require(s_randomWords[0] != 0, "Error: RANDOM NUMBER NOT RECEIVED YET");

        vrfRandomNumber = s_randomWords[0];
    ...
    function initGame() onlyOwner public {
        ...
        if(!testingMode){
            // make VRF request for a new random number
            s_requestId = COORDINATOR.requestRandomWords(
                keyHash,
                s_subscriptionId,
                requestConfirmations,
